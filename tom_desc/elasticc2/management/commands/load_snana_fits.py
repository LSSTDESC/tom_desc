...
class FITSFileHandler( ColumnMapper ):
    ...
    def load_one_file( self, headfile, photfile ):
        try:
            ...
            if self.really_do:
                nobj = DiaObject.bulk_insert_or_upsert( dict( head ) )
            ...
            if self.really_do:
                nfrc = DiaForcedSource.bulk_insert_or_upsert( phot )
            ...
            if self.really_do:
                nsrc = DiaSource.bulk_insert_or_upsert( phot )
            ...
            if self.really_do:
                nalrt = Alert.bulk_insert_or_upsert( alerts )
